...
static void build_config_path(Config *cfg)
{
    ...
    snprintf(cfg->config_path, sizeof(cfg->config_path),
             "%s/%s/%s", home, CONFIG_DIR, CONFIG_FILE);
}
...
static bool write_defaults(const char *path)
{
    ...
    fprintf(fp,
            ...
            "address=%s\n"
            "interval=%d\n",
            DEFAULT_ADDRESS, DEFAULT_INTERVAL);
    fclose(fp);
    return true;
}
                             ...
                             gpointer           user_data G_GNUC_UNUSED)
{
    if (event == G_FILE_MONITOR_EVENT_CHANGED ||
        ...
        GCallback cb = G_CALLBACK(g_object_get_data(G_OBJECT(monitor), "user-callback"));
...
void config_watch(Config *cfg, GCallback callback, gpointer user_data)
{
    ...
    g_object_set_data(G_OBJECT(cfg->monitor), "user-callback", (gpointer)callback);
    g_object_set_data(G_OBJECT(cfg->monitor), "user-data", user_data);

    cfg->monitor_handler_id = g_signal_connect(cfg->monitor, "changed",
